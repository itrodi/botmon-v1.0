                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <h2 className="text-lg font-semibold">
                      Notifications
                      {notifications.filter(n => !n.marked).length > 0 && <span className="ml-2 bg-purple-600 text-white text-xs px-2 py-1 rounded-full">{notifications.filter(n => !n.marked).length} new</span>}
                    </h2>
                      ...
                      {notifs.map(notification => (
                        <div key={notification.id || notification.ids || Math.random()} className={`flex items-center gap-4 p-4 hover:bg-gray-50 rounded-lg transition-colors cursor-pointer ${!notification.marked ? 'bg-purple-50 hover:bg-purple-100' : ''}`} onClick={() => !notification.marked && markAsRead(notification.id || notification.ids)}>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            {markingAsRead[notification.id || notification.ids] ? <div className="w-4 h-4 animate-spin rounded-full border-2 border-purple-600 border-t-transparent"></div> : notification.marked ? <CheckCircle2 className="w-4 h-4 text-green-600" /> : getNotificationIcon(notification)}
                          </div>
                        </div>
                      ))}
                    </div>
