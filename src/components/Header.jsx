...
const NotificationPage = () => {
  ...
  const getNotificationTitle = (notification) => {
    ...
    if (type === 'User in 24 hours') return `${customerName} messaged you`;
    return notification.title || notification.subject || `${type} from ${customerName}`;
  };

  const getNotificationMessage = (notification) => {
    if (notification.message || notification.body || notification.description) return notification.message || notification.body || notification.description;
  ...
  // Get profile picture: use profile_picture from notification data, fallback to generated avatar
  const getProfilePicture = (notification) => {
    // Check for profile picture in the notification data
    if (notification.profile_picture) return notification.profile_picture;
    if (notification.profilePicture) return notification.profilePicture;
    if (notification.avatar) return notification.avatar;
    if (notification.image) return notification.image;
    // Fallback to generated avatar
    const name = notification.name || notification.username || 'User';
    const color = getAvatarColor(notification);
  ...
  const processNotification = (notif) => ({
    ...
    profilePic: getProfilePicture(notif),
  });
  ...
  // ── Mark notification as read — POST to /mark-notifications ──
  const markAsRead = async (notificationId) => {
    ...
    try {
      ...
      setNotifications(prev =>
        prev.map(n =>
          (n.id === notificationId || n.ids === notificationId || n._id === notificationId)
            ? { ...n, marked: true }
            : n
        )
      );
  ...
  const markAllAsRead = async () => {
    ...
    try {
      ...
      // Mark each one (or send all IDs if backend supports array)
      const response = await fetch('https://api.automation365.io/mark-notifications', {
        method: 'POST',
      ...
      if (response.ok) {
        setNotifications(prev => prev.map(n => ({ ...n, marked: true })));
  ...
  const getAvatarColor = (notification) => {
    ...
    const colors = { 'product': 'f97316', 'order': 'f97316', 'new_customer': '6d28d9', 'successful_transaction': '22c55e', 'failed_transaction': 'ef4444', 'new_chat': '3b82f6', 'payment_received': '22c55e', 'customer': '6d28d9', 'chat': '3b82f6', 'user in 24 hours': '3b82f6' };
  ...
  const groupNotificationsByDate = () => {
    ...
    filterNotifications().forEach(notif => {
      try { const d = new Date(notif.timestamp); const key = isToday(d) ? 'Today' : isYesterday(d) ? 'Yesterday' : format(d, 'MMMM d, yyyy'); if (!grouped[key]) grouped[key] = []; grouped[key].push(notif); }
      catch { if (!grouped['Unknown']) grouped['Unknown'] = []; grouped['Unknown'].push(notif); }
    });
    return grouped;
  };

  const unreadCount = notifications.filter(n => !n.marked).length;
  const filteredNotifications = filterNotifications();
  const groupedNotifications = groupNotificationsByDate();

  if (loading) return (<div className="flex h-screen bg-gray-50"><Sidebar /><div className="flex-1 flex items-center justify-center"><div className="text-center"><div className="animate-spin rounded-full h-12 w-12 border-b-2 border-purple-600 mx-auto mb-4"></div><p className="text-gray-500">Loading notifications...</p></div></div></div>);

  if (error) return (<div className="flex h-screen bg-gray-50"><Sidebar /><div className="flex-1 flex flex-col overflow-hidden"><Header title="Notifications" /><div className="flex-1 flex items-center justify-center"><div className="text-center"><AlertCircle className="w-12 h-12 text-red-500 mx-auto mb-4" /><p className="text-gray-700 mb-2">Error loading notifications</p><p className="text-gray-500 text-sm mb-4">{error}</p><div className="flex gap-2 justify-center"><Button onClick={() => fetchNotifications()} variant="outline">Try Again</Button></div></div></div></div></div>);
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <h2 className="text-lg font-semibold">
                      Notifications
                      {unreadCount > 0 && <span className="ml-2 bg-purple-600 text-white text-xs px-2 py-1 rounded-full">{unreadCount} new</span>}
                    </h2>
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => fetchNotifications(true)} disabled={isRefreshing} className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
                  ...
                  <div key={dateGroup}>
                    ...
                    <div className="bg-white rounded-lg shadow-sm divide-y divide-gray-100">
                      {notifs.map(notification => {
                        const notifId = notification.id || notification.ids || notification._id;
                        return (
                          <div
                            ...
                            onClick={() => !notification.marked && notifId && markAsRead(notifId)}
                          >
                            {/* Profile picture */}
                            <div className="w-12 h-12 rounded-full overflow-hidden flex-shrink-0">
                              <img
                                ...
                                onError={(e) => {
                                  const name = notification.name || notification.username || 'User';
                                  const color = getAvatarColor(notification);
                                  e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=${color}&color=fff&size=96`;
                                }}
                              />
                            </div>
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-gray-500 text-sm truncate">{notification.displayMessage}</p>
                              <div className="flex items-center gap-2 mt-1">
                                {notification.platform && <span className={`text-xs px-2 py-0.5 rounded-full capitalize ${notification.platform.toLowerCase() === 'instagram' ? 'bg-pink-100 text-pink-700' : notification.platform.toLowerCase() === 'whatsapp' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>{notification.platform}</span>}
                                {notification.status && <span className={`text-xs px-2 py-0.5 rounded-full ${notification.status === 'Confirmed' ? 'bg-green-100 text-green-700' : notification.status === 'Rejected' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>{notification.status}</span>}
                              </div>
                            </div>
                            <div className="flex items-center gap-3">
                              <span className="text-sm text-gray-500 hidden sm:block">{formatTimestamp(notification.timestamp)}</span>
                              {markingAsRead[notifId] ? <div className="w-4 h-4 animate-spin rounded-full border-2 border-purple-600 border-t-transparent"></div> : notification.marked ? <CheckCircle2 className="w-4 h-4 text-green-600" /> : getNotificationIcon(notification)}
                            </div>
                          </div>
                        );
                      })}
              ...
              <div className="lg:col-span-1">
                <div className="bg-white p-6 rounded-lg shadow-sm space-y-4">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between"><div className="flex items-center gap-2"><Mail className="w-5 h-5 text-purple-600" /><span>Email</span></div><Switch checked={notificationChannels.email} onCheckedChange={() => setNotificationChannels(p => ({ ...p, email: !p.email }))} /></div>
                    <div className="flex items-center justify-between"><div className="flex items-center gap-2"><MessageSquare className="w-5 h-5 text-green-600" /><span>SMS</span></div><Switch checked={notificationChannels.sms} onCheckedChange={() => setNotificationChannels(p => ({ ...p, sms: !p.sms }))} /></div>
                    <div className="flex items-center justify-between"><div className="flex items-center gap-2"><MessageSquare className="w-5 h-5 text-blue-600" /><span>WhatsApp</span></div><Switch checked={notificationChannels.whatsapp} onCheckedChange={() => setNotificationChannels(p => ({ ...p, whatsapp: !p.whatsapp }))} /></div>
                    <div className="flex items-center justify-between"><div className="flex items-center gap-2"><Phone className="w-5 h-5 text-purple-600" /><span>Call</span></div><Switch checked={notificationChannels.call} onCheckedChange={() => setNotificationChannels(p => ({ ...p, call: !p.call }))} /></div>
                  </div>
                  <div className="pt-4 border-t">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between"><span className="text-gray-500">Unread</span><span className="font-medium text-purple-600">{unreadCount}</span></div>
